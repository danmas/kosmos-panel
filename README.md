...
- **GET-FILE директива**: skills могут читать файлы проекта напрямую через `[GET-FILE] path` без shell-команд
- **Настройка истории LLM**: per-skill настройка `history_mode` (full/last_n/system_only) для контроля передачи истории в LLM
...
**Как работает:** Сервер отслеживает изменения файла `inventory.json` через `fs.watchFile` (интервал 1с). При любом сохранении файла автоматически:
1. Перечитывает `inventory.json`
2. Запускает повторный опрос всех сервисов (`pollOnce`)

**Не требует перезапуска сервера.** Править `inventory.json` можно в любом редакторе — изменения подхватываются в реальном времени.
