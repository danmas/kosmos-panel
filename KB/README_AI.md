...
Для повышения точности ответов AI в терминале, система подгружает дополнительный контекст (знания) из специального файла на **удалённом сервере**, к которому вы подключаетесь по SSH.

**Приоритет поиска файла знаний:**

1.  **`./.kosmos-panel/kosmos-panel.md`** (приоритет 1)
    -   **Расположение**: В текущей рабочей директории на удалённом сервере.
    -   **Назначение**: Контекст, специфичный для конкретного проекта или директории. Например, структура проекта, используемые команды сборки, пути к логам.

2.  **`~/.config/kosmos-panel/kosmos-panel.md`** (приоритет 2, fallback)
    -   **Расположение**: В стандартной директории конфигов пользователя на удалённом сервере.
    -   **Назначение**: Общий контекст для всего сервера. Например, имена сервисов, IP-адреса, стандартные процедуры.

**Логика работы:**
-   При выполнении `ai:`-команды система пытается прочитать файл по приоритету 1.
-   Если файл не найден — используется файл по приоритету 2.
-   Если оба файла отсутствуют — AI работает без дополнительного контекста.
-   Содержимое найденного файла добавляется в системный промпт AI.

**Пример содержимого `kosmos-panel.md`:**
```markdown
# Контекст сервера prod-api

## Важные пути
- Логи приложения: /var/log/myapp/
- Конфиг nginx: /etc/nginx/sites-enabled/

## Команды
- Рестарт сервиса: sudo systemctl restart myapp
- Проверка статуса: sudo systemctl status myapp
```
