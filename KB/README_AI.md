...
### AI Health Check (проверка доступности AI при запуске терминала)

При запуске терминала (в любой точке входа: workspace, автономный терминал, оверлей с дашборда) система автоматически проверяет доступность AI-модели. Если AI не отвечает — пользователь получает предупреждающий `alert`.

**Точки вызова:**
- `web/workspace.js` — через 1.5с после создания первого терминала (флаг `_aiChecked` предотвращает повторный alert при сплите панелей)
- `web/term.html` — в `ws.onopen` standalone-терминала
- `web/app.js` — в `ws.onopen` оверлейного терминала (флаг `_aiCheckedApp`)

**Endpoint:** `POST /api/ai-test` (реализован в `server.js`)
- Принимает опционально: `{ baseUrl, model, prompt }` (если не переданы — берутся из `.env`)
- Отправляет короткий OpenAI-совместимый запрос: `max_tokens: 32`, `temperature: 0`, таймаут 15с
- Возвращает: `{ success: true/false, response, model, baseUrl, latencyMs, error? }`
- Если `success: false` — показывается `alert('⚠️ AI не отвечает: <причина>')`

**Кнопка ручной проверки в Inventory Editor:**
- Вкладка «AI Config» (`/inventory-editor-react.html`) содержит кнопку «Проверить AI»
- Отправляет текущие (несохранённые) значения полей `AI_KOSMOS_MODEL_BASE_URL` и `AI_MODEL` на `/api/ai-test`
- Результат отображается встроенным блоком с ответом модели, задержкой и метаданными

**Реализация:**
- Backend: `server.js` → `POST /api/ai-test` (строки ~654–745)
- Frontend: `web/inventory-editor-react.html` → `AiConfigTab.testAi()` (строки ~237–271), `web/workspace.js` → `checkAiHealth()` (строки ~2133–2156), `web/term.html` → `checkAiHealth()` (строки ~1339–1362), `web/app.js` → `checkAiHealthApp()` (строки ~319–340)
