...
- Опционально в начало системного промпта добавляется контекст с **удалённой машины**: содержимое `./.kosmos-panel/kosmos-panel.md` или `~/.config/kosmos-panel/kosmos-panel.md` (читается по SSH с той же сессии).
...
- **Контекстные файлы на удалённой машине** (`.kosmos-panel/kosmos-panel.md`, `~/.config/kosmos-panel/kosmos-panel.md`) используются только для **терминального AI** (п.1) и опционально для **скиллов** (п.2).
