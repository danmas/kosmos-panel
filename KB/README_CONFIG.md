...
### Перезагрузка AI-конфигурации (config.json)
...
### Горячая перезагрузка inventory.json

**Механизм:** Сервер отслеживает изменения файла `inventory.json` через `fs.watchFile()` (интервал 1с). При любом сохранении файла автоматически срабатывает последовательность:

1. `reloadInventory()` — перечитывание и парсинг `inventory.json`
2. `pollOnce()` — запуск повторного опроса всех сервисов с новыми настройками

**Не требует никаких действий** — просто сохрани файл. Изменения применяются автоматически, старые сервисы заменяются новыми без перезапуска сервера.

**Проверка:** через 1-2 секунды после сохранения обновлённые данные появятся в API (`GET /api/inventory`) и на панели мониторинга.

**Реализация:** `server/monitor.js`, строка 431:
```javascript
