...
- **Результат удалённой команды (REST Bridge)**:
  ```json
  { "type": "command_result", "commandId": "uuid", "status": "completed", "stdout": "...", "stderr": "", "exitCode": 0 }
  ```
  Отправляется клиентом после выполнения команды, полученной через REST API.
...
- **Session ID (REST Bridge)**:
  ```json
  { "type": "session", "sessionId": "0ec45016-b1ae-4d88-94ee-979d8e3cb9de" }
  ```
  Отправляется сразу после успешного подключения. Используется клиентом для отображения ID и для REST API запросов.

- **Удалённая команда (REST Bridge)**:
  ```json
  { "type": "remote_command", "commandId": "uuid", "command": "ls -la", "requireConfirmation": true }
  ```
  Команда, отправленная через REST API. Клиент должен её выполнить или показать панель подтверждения.

- **Отмена команды (REST Bridge)**:
  ```json
  { "type": "cancel_command", "commandId": "uuid" }
  ```
  Отмена ранее отправленной команды через REST API.
...
Kosmos Panel предоставляет два типа терминальных REST API:

#### 1. Terminal REST API (v1/v2)

Создаёт **отдельные SSH-сессии** для выполнения команд. Подходит для автоматизации без визуального интерфейса.

- **v1** - базовая версия с простыми JSON-ответами
- **v2** - улучшенная версия со стандартизированным форматом ответов

**Основные операции:**
1. Создание сессии - устанавливает SSH-соединение с сервером
2. Выполнение команды - отправляет команду на выполнение
3. Закрытие сессии - завершает SSH-соединение

#### 2. WebSocket Terminal REST Bridge API (NEW)

Управляет **браузерными терминалами** через REST. Позволяет:
- Отправлять команды в терминалы, открытые пользователем в браузере
- Получать подтверждение от пользователя перед выполнением опасных команд
- Синхронно или асинхронно получать результаты выполнения

**Ключевые особенности:**
- Пользователь видит выполняемые команды в реальном времени
- Опциональное подтверждение команд
- Session ID отображается в браузере для удобства

**Эндпоинты:**
- `GET /api/ws-terminal/sessions` - список активных терминалов
- `POST /api/ws-terminal/:sessionId/command` - отправка команды
- `GET /api/ws-terminal/command/:commandId` - статус команды
- `DELETE /api/ws-terminal/command/:commandId` - отмена команды

### Когда использовать какой API?

| Сценарий | Рекомендуемый API |
|----------|------------------|
| Автоматизация без UI | Terminal REST API v2 |
| Скрипты CI/CD | Terminal REST API v2 |
| Команды с подтверждением пользователя | WS Terminal REST Bridge |
| Интерактивная работа с пользователем | WS Terminal REST Bridge |
| AI-агент с визуализацией | WS Terminal REST Bridge |

### Реализация

- **Terminal REST API**: `server/terminal.js`
- **WS Terminal Bridge**: `server/ws.js` + `server/index.js`
- **Клиентская часть Bridge**: `web/term.html`
