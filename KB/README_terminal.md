...
При подключении по SSH система определяет тип ОС (`uname -s`). Результат кэшируется и добавляется в промпт:
- **Linux**: `ls`, `cat`, `grep`, `find`, `systemctl`
- **macOS**: POSIX/BSD команды
- **Windows**: `dir`, `type`, `findstr`, `tasklist`, PowerShell

Это позволяет AI генерировать команды для правильной платформы.
...
3. Сервер в `ws.js`:
   ...
   - Добавляет в промпт информацию о целевой ОС (см. выше).
